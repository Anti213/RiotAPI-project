...
def makeParticipantDictionaries(data, itemRef, championRef):
	...
	for participant in data['participants']:
		for itemId in itemIds:
			if(str(participant['stats'][itemId]) in itemRef):
				pprint(itemRef[str(participant['stats'][itemId])])
				participantIds.append(participant['participantId']-1)
				break
	...
	for participantId in participantIds:
		...
		# making new directory for item names... making sure to error check!
		championDict['participants']['stats']['itemNames'] = dict()
		for itemName in itemIds:
			itemId = str(championDict['participants']['stats'][itemName])
			itemId_name = 'itemName' + itemName[-1:]
			#print(newItemId)
			#print(itemName)

			if (itemId in itemRef):
				#print(itemId + ': ' + itemRef[itemId])
				championDict['participants']['stats']['itemNames'][itemId_name] = itemRef[itemId]
			else:
				championDict['participants']['stats']['itemNames'][itemId_name] = 'N/A'

		#pprint(itemRef)
		# writing to file
		# writing to a specific directory
		
		fileName1 = championName[0].lower() + '.json'
		directory = '/home/lenneth/Documents/RiotAPI project/parsed champion data/'
		fileName = directory + fileName1
		#os.path.join('/home/lenneth/Documents/RiotAPI project/parsed match data',
		#		     fileName)
